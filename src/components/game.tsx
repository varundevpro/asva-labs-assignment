...
export function Game() {
  ...
  // Use useEffect to trigger the roll animation when the value changes
  React.useEffect(() => {
    if (currentXp !== null) {
      setIsRolling(true);
  ...
  React.useEffect(() => {
    if (currentXp != null && !isRolling) confettiRef.current?.fire({});
  }, [isRolling, currentXp]);
  ...
  const rollDice = () => {
    ...
    const xp = (Math.floor(Math.random() * 6) + 1) * 10; // Generates a number between 1 and 6
    setCurrentXp(xp);
    addEntryToHistory(xp);
  };

  return (
    <div className="relative bg-white p-4 flex flex-col gap-2 flex-1/3 justify-between items-center rounded-xl shadow-lg">
      <Confetti
        ref={confettiRef}
      ...
      <div className="text-center">
        {isRolling ? (
          <p className="text-5xl font-medium">Rolling..</p>
        ) : currentXp ? (
          <p className="text-5xl font-medium text-green-700">+{currentXp} XP</p>
        ) : (
          diceIcon
        )}
      </div>
      <button
        type="button"
        className="text-white bg-blue-700 hover:bg-blue-800 focus-visible:ring-4 focus-visible:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 focus-visible:outline-none disabled:opacity-50 disabled:pointer-events-none disabled:cursor-not-allowed active:scale-95"
        onClick={rollDice}
        disabled={isRolling}
      >
        Roll dice
